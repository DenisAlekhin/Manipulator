package app.globalsearch.singledimensional;
...
public class OneDimensionalGlobalSearchWithLimitations {
    ...
    private double calcGv(double point, int v) throws Exception{
        switch(v) {
            ...
            case 3: {
                for (Pair<Double, Double> pair : analysis) {
                    if (pair.getKey() == point) {
    ...
    private Pair<Double, Double> getResult() throws NoSolutionExceptions{
        ArrayList<Integer> Iv = new ArrayList<>(I.get(3));
        ArrayList<Pair<Double, Double>> result = new ArrayList<>(Iv.size());
        for (Integer i : Iv) {
